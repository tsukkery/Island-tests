...
public class AnimalEatTask implements Runnable {
    ...
    public void run() {
        ...
        if (countAnimalsStart > 0 && !animals.stream().filter(c -> !(c.getName().equals("Caterpillar"))).toList().isEmpty()) {
            ...
            System.out.printf("ПОБЕДИЛИ ГУСЕНИЦЫ! В ЖИВЫХ ОСТАЛИСЬ ТОЛЬКО ОНИ НА %d ДЕНЬ!", StatisticsTask.getCurrentDay()); // они бесссмертные и не требуют пищи
            IslandSimulation.getInstance().getExecutorService().shutdown();
            System.exit(0);
        }
        latch.countDown();
